...
class DQNAgent():
    ...
    def act(self, state) -> Tensor:
        ...
        if choice > self.epsilon:
            with torch.no_grad():
                out = self.policy_model(state.to(self.device)).max(0).indices.view(1)
                return out
        else:
    ...
    def replay(self):
        ...
        expected_Q = reward_batch + (settings.gamma * future_rewards * (1-done_batch))
    ...
    def update_target(self):
        for target_param, local_param in zip(self.target_model.parameters(), self.policy_model.parameters()):
